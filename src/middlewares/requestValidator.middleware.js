...
const { errorCode: {
    VALIDATION_ERROR,
    INVALID_ROUTE,
    INVALID_METHOD } } = require('../constants');

exports.requestValidator = (req, res, next) => {
    const { method, body, files, _parsedUrl, query, baseUrl, url, originalUrl } = req;
    ...
    try {
        if (!validationSchemas[fullURL])
            throw errorResponse(INVALID_ROUTE);
        ...
        if (error)
            throw { ...error.details[0], code: VALIDATION_ERROR };
        else
            next();
